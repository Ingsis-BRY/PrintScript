...
class InterpreterTest {
    ...
    private fun binary(left: Expression, operator: BinaryOperator, right: Expression) =
        Expression.BinaryExpression(left, operator, right, at, at)

    private fun declare(name: String, type: Type, initializer: Expression?) =
        Statement.VariableDeclaration(name, type, initializer, at, at)

    private fun assign(name: String, value: Expression) =
        Statement.Assignment(name, value, at, at)

    private fun println(argument: Expression) =
        Statement.CallStatement("println", argument, at, at)
